¿Que hace un programador?
-------------------------

El trabajo de un programador de software es en general
**escribir instrucciones en archivos de texto simples** en un lenguaje de programación específico.  
Un programador conoce uno o más *lenguajes de programación* y puede desempeñarse con ellos en
múltiples entornos de trabajo.  
Estos entornos incluyen:

 - Una página web
 - Una aplicación para tu celular
 - Un sistema que funciona en una computadora estándar (como Word, Google Chrome, Excel o
   cualquier programa que ves en el *escritorio* de tu computadora)
 - Micro-sistemas que sirvan de soporte a sistemas más grandes y que *no se ven*

Acotar estos entornos donde un software se ejecuta es difícil, son múltiples.  

.. image:: /_static/imgs/hola1.png
